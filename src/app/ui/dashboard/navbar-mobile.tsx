...
export function NavbarMobile() {
    ...
    return (
        <nav className='max-h-full w-[95%] rounded-full mb-2 relative  bg-gradient-to-b from-jungle-green-400 to-jungle-green-600 '>
    ...
    return (
        <div className="w-full">
            <Link key={redirect} href={redirect}>
                <li className={clsx('w-full flex gap-2 p-2 content-start text-center hover:text-black active:text-jungle-green-500 hover:bg-white active:bg-white rounded-full transition-colors justify-center', { 'bg-white text-jungle-green-500': pathname === redirect })}> <i className={icon}></i> <p className="hidden lg:block truncate">{title}</p></li>
            </Link >
        </div>
    )
}