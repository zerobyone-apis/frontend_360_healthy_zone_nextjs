...
export default function TrainingView({ setCurrentIndex, exercise }: Props) {
	...
	useEffect(() => {
		...
		if (exercise.duration_in_seconds) {
			timerout = setTimeout(() => {
				halfTimeSoundEffect.play();
			}, (exercise.duration_in_seconds * 1000) / 2 || 0);
		}
	...
	return (
		<div
			className="h-full p-4 rounded w-full flex justify-center flex-col items-center gap-3 overflow-auto"
			aria-labelledby="drawer-label"
		>
			<h1 className="text-2xl font-bold text-gray-900 text-center capitalize">
				{exercise.name}
			</h1>
			<h5 className="text-3xl text-jungle-green-500 font-bold">
			...
			<Image
				...
				width={318}
				height={159}
				className="rounded border border-jungle-green-500"
			></Image>
			{timeLeft ? (
				<h2 className="text-gray-900 text-3xl font-bold">
					Time left: <span className="text-jungle-green-500">{timeLeft}</span>
				</h2>
			) : (
				<h2 className="text-gray-900 text-xl font-bold text-center">
					Finish all the sets and press done
				</h2>
			)}
			<div className="flex gap-3">
				<Button
					onClick={() => router.push("/dashboard/trainings")}
					className="text-gray-900 border border-jungle-green-500 rounded"
				>
					Need a break
				</Button>
				<Button
					onClick={handleReady}
					className="text-white bg-jungle-green-500 rounded"
				>
					READY
				</Button>
			</div>
