...
export default function Page({}: Props) {
  ...
  const handleSubscription = useCallback(
    async (planId: string, price: number, type: string) => {
      setLoading(true);
      try {
        if (!clientId) {
          toast.error('No valid client ID found. Please login again.');
          setLoading(false);
          return;
        }
      ...
      pollIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (result.success && result.user) {
            ...
            if (
              result.user.client?.subscription &&
              result.user.client?.subscription.status == 'ACTIVE'
            ) {
  ...
  useEffect(() => {
    ...
    if (user.client?.subscription.status != "ACTIVE" && plan_id && price && plan_type) {
              ...
              <button
                ...
                onClick={() => {
                  const confirmed = window.confirm(
                    'Are you sure you want to cancel? Your payment process will be interrupted.'
                  );
                  if (confirmed) {
                    setPaypalLink(null);
                  }
