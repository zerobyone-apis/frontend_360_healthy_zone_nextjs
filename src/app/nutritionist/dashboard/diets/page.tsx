        ...
        getDashboardStats()
            .then((data: any) => {
                ...
                data.goals_created.forEach((goal: any) => {
                    const complete_diet_list = goal.diets.map((diet: any) => {
                        return { ...diet, client_info: { ...goal.client } };
                    });
                    setDiets([...diets, ...complete_diet_list]);
                });
            })
            .catch(e => {
                console.error(e);
    ...
    return (
        <section>
            ...
            <div className="gap-3 flex flex-col">
                {diets.map((diet: any, index: number) => <DietResumeCard key={diet.diet_id} diet={diet} index={index} />)}
            </div>
